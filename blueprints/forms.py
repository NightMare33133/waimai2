...
class QuestionForm(wtforms.Form):
    ...
    content = wtforms.StringField(validators=[length(min=2)])

class AnswerForm(wtforms.Form):
    content = wtforms.StringField(validators=[length(min=1)])

class WaiMaiForm(wtforms.Form):
    dish = wtforms.StringField(validators=[length(min=1, max=200)])
    address = wtforms.StringField(validators=[length(min=1, max=200)])
    telephone = wtforms.StringField(validators=[length(min=1, max=200)])